...
const BookingListing = ({ columns, rows }) => {
  ...
  const flattenedBookingDataWithTotal = FilteredRows.map((item) => {
    const total = item.total; // Assuming 'total' is a property of the item or its sub-object
    const bookingData = item.bookingData; 
    const id=item.id// Assuming 'bookingData' is the array you want to flatten
  const status=item.status
    const flattenedBookingData = bookingData.map((bookingItem) => {
      return {
        ...bookingItem,
        total,
        id,status
    ...
    return flattenedBookingData;
  }).flat();
  
  console.log(flattenedBookingDataWithTotal);
    ...
    <div className="flex flex-col gap-2 px-4 py-3 h-full">
      ...
      <FormSectionHeading>Bookings</FormSectionHeading>
        ...
        <div className="border flex flex-col grow overflow-auto rounded bg-white">
          <CMPTable
            columns={columns}
            rows={flattenedBookingDataWithTotal}
            isCheckbox={false}
